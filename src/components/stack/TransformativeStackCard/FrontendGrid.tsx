      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {frontendSkills.map((skill) => (
            <div 
              key={skill.name}
              className="p-4 rounded-2xl border border-white/10 bg-white/5 dark:bg-zinc-900/50 hover:border-accent-hover/20 hover:bg-white/10 dark:hover:bg-accent-hover/20 transition-all duration-300 group flex flex-col justify-between h-23 shadow-sm hover:shadow-lg"
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-muted-foreground group-hover:text-foreground/40 transition-colors">
                  {skill.level}%
                </span>
              </div>
